...
**16 · Roadmap (~90s).** "Low-risk ramp — the prototype already encodes the hard parts. Pilot: real auth, Secret Manager, hardening, instrumentation. Integrate: live OMS/payments, CCAI telephony, Vertex Translation. Scale: multi-region, evals in CI, more sub-agents. Each phase is integration + hardening, not re-architecture."

**17 · Close + Q&A (~30s, then open).** "To recap: one agent across every channel, language and modality; safe to transact because deterministic code does the transacting; and it heals itself. It directly unblocks the engagement and it's native to Google Cloud. I'd love your questions — I can pull up the extension or `/dev` to answer any of them." *(Keep `QUESTION-ANSWER.md` handy.)*
